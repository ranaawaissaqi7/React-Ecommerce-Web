...
import { isShipingAdressChangeHandler } from '../../store/features/AuthSlice';
export default function ShipingAdress() {
  ...
  useEffect(() => {
    ...
    const getUserSignUpData = async () => {
      try {
        ...
        console.log(" UserSignupData => ", res)
        setUserSignUpData(res)
      } catch (error) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      values.uuid = userSignUpData.uuid
  ...
  const storeDataLocalStorage = (lowercaseValues) => {
    try {
      ...
      disPatch(isShipingAdressChangeHandler(true))
          ...
          <div className="grid grid-cols-1">
            <div className="left-item md:order-1 order-2 flex flex-col justify-center">
              <div className="text-content text-center">
                <h1 className=" font-bold [font-size:25px] my-5">Add Shiping Adress</h1>
              </div>
              {/* form */}
