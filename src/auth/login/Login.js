...
export default function Login() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      password: Yup.string()
        .trim()
        .min(7, 'Enter minimum password length 7 characters')
    ...
    onSubmit: (values) => {
      ...
      const lowercaseValues = Object.keys(values).reduce((acc, key) => {
        acc[key] =
          typeof values[key] === 'string' ? values[key].toLowerCase() : values[key];
        return acc;
      }, {});
      // console.log("lowerCase => ",lowercaseValues)
      //  console.log("signUp Values => ",userSignUpDataState)
      if (!userSignUpDataState) {
        return setUserSignUpDataState("User Not Found Please go to SignUp Page ")
      }
      // login credentials
      if (userSignUpDataState.email === lowercaseValues.email) {
        if (
          userSignUpDataState.email === lowercaseValues.email &&
          userSignUpDataState.password === lowercaseValues.password
        ) {
                ...
                <div className="flex justify-center">
                  <div className="lg:basis-3/5 basis-11/12">
                    <p className="my-4 text-center">
                      ...
                      <Link to="/signUp" className="text-green-800 underline hover:opacity-80 ms-1">
                        Sign Up here!
                      </Link>
                    </p>
                    ...
                    {/* <!-- Error Alert --> */}
                    {credentialsError ? (
                      <div className="font-bold text-red-600 [font-size:18px] text-center">
                        {credentialsError}
                      </div>
                    ) : null}

                    <button
                      className="bg-green-800 text-white rounded p-4 my-5 hover:opacity-75 [width:100%]"
                      type="submit"
                    >
                      Sign In
                    </button>
